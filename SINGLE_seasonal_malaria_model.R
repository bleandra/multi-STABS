setwd("~/OneDrive - The University of Melbourne/Rithmetic")
source('~/OneDrive - The University of Melbourne/Rithmetic/mozzy_model.R')
...
#simulate the full epidemic for up to 30 days
max_t<-365*2
...
#time/counter
t<-0
#t_vec<-matrix(NA,nrow=10000,ncol=1)
#t_count<-1
#t_vec[t_count]<-t


#the sick people (with plasmodium falciparum)
sick_puppies<-which(ind_level_data$malaria==1)
...
human_state[sick_puppies]<-1
initial_I<-length(sick_puppies)
#initial recovery times
#of humans
for( jj in 1:length(sick_puppies)){
  h_stack_count<-h_stack_count+1
  human_stack$hh_id[h_stack_count]<-ind_level_data$hh_id[sick_puppies[jj]]
  ...
  #t<-rexp(1,rate=sigmaH)
  
  #new_mozzys<-ind_level_data$m_mosquito_rate[sick_puppies[jj]]
  ...
  while((t2+t_delta)<min(tR[jj],max_t)){
    ...
    #t_delta<-rexp(1,rate=total_new_mozzy_rate)
    ...
    #ind_moz_index<-sick_puppies[which((runif(1)*total_new_mozzy_rate)<cum_new_mozzys)[1]]
    ...
    while(temp_t<temp_death_time[m_count] & temp_state!=3 & temp_t<max_t){
      ...
      if(temp_state==1){
        ...
        hhprobs<-(seasonal_MAP_hh[,the_season]*hh_bite_prop)*(pnorm(Distances[,temp_mozzy_hh_index],0,10/3)/pnorm(0,0,10/3))
        #hhprobs<-(hh_import_rate[,3]*hh_bite_rate)/Distances[,temp_mozzy_hh_index]
